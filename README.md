...
当前版本支持编导手工把语音片段映射到角色，但仍不能自动判断说话人身份，也不能根据画面判断动作、表情、服装和道具；OCR 只读取硬字幕，画面扫描只读取技术信号，都不等于理解视频语义。异常均为“待编导复核”，不得冒充最终质量结论。完整视觉演进见 `MULTIMODAL_ROADMAP.md`，真实媒体样本的指标定义和局限见 `docs/ACTUAL_MEDIA_EVALUATION.md`。
