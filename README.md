# ScriptLint（短剧成片多模态审片与反馈记忆 Agent）
...
当前 V4 增加 RapidOCR 字幕证据：按自适应时间间隔抽取视频下半屏画面，读取硬字幕并与 ASR、剧本三方比对。字幕与剧本一致，且同时间段存在实际语音时，即使 ASR 严重错字也可标记为“字幕强证据通过”；如果没有任何语音证据，则进入“待听确认”，不冒充通过，也不直接计作漏词。
...
当前 V9 加入审片人工工作台：可导入 SRT/VTT（有字幕文件时自动跳过 OCR）、导出修订后的 SRT、修改或新增 ASR 时间码片段，并填写声纹标签和角色映射；应用修订时只重跑轻量文本对齐。台词和画面问题均可一键把播放器定位到对应时间码。当前是人工说话人映射，不冒充自动声纹识别。

当前 V10 加入轻量说话人声学分组：系统按 ASR 时间段从临时 WAV 提取 MFCC/频谱特征，自动估计或按编导指定人数生成“声学组 A/B/C”，再以剧本角色台词作弱监督给出角色候选。只有角色多数证据达到 78% 才自动写入，达到更严格门槛才提示台词归属冲突；低置信结果必须由编导在声音组工作台确认。该能力不保存声纹模板，也不等于现实身份识别。
...
文本侧继续支持 `.txt / .md / .fountain`、项目版本、人物事实和反馈记忆。工作台包含“视频音频审片 / 剧本审计 / 项目规则 / 固定评测 / 用户验证 / 演进路线”六页。推荐现场路径：粘贴带“角色：台词”的剧本 → 上传已获授权的单集或片段 → 可选导入 SRT/VTT → 运行审核 → 回看时间码 → 修订 ASR/映射角色 → 提交编导纠正并确认规则 → 后续版本复用。

当前版本支持自动声音相似分组和编导确认角色，但仍不能判断现实说话人身份，也不能根据画面判断动作、表情、服装和道具；OCR 只读取硬字幕，画面扫描只读取技术信号，都不等于理解视频语义。异常均为“待编导复核”，不得冒充最终质量结论。完整视觉演进见 `MULTIMODAL_ROADMAP.md`，真实媒体样本的指标定义和局限见 `docs/ACTUAL_MEDIA_EVALUATION.md`。
